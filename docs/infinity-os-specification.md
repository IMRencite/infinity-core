...
| **Autonomous Plan Execution Integration v1** | `plan_executions` coordination record; Mission Runtime v2 connects allocation → scheduling → execution → review via bounded ticks; zero-cost allocation reuse; Scheduler batches plan steps; Build Factory v2 for `build` steps; `qa.verify_autonomous_plan_execution`; unsupported external steps (`blocked_external_capability_required`); HQ read-only execution summary; **no deployment, publishing, domains, hosting, repos, purchases, or unrestricted shell/network** |
| **Venture Assembly Foundation v1** | `venture_assemblies` + external dependency registry; consumes internally_complete PlanExecution and reproducible build snapshots; packages (identity, business model, brand, digital property refs, monetization, GTM, operations, legal placeholders); `venture.assemble_internal_package` + `qa.verify_venture_assembly`; launch-readiness (`internally_ready` ≠ live); links `companies` draft venture; HQ Ventures + mission inspector **8c**; **no launch, deploy, domains, payments, or external accounts** |
| **Launch & Deployment Gateway Foundation v1** | `external_actions`, `launch_plans`, `external_resources`, approvals; **External Action Gateway**; mock + GitHub + Vercel adapters; `launch.generate_plan`, `launch.simulate_external_action`, `launch.execute_external_action` (gated live); live scope: repo create/push, hosting create/deploy/verify; env gates default false; HQ External Resources; simulation E2E + `validate:live-provider-e2e` fail-closed |
