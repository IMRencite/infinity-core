...
if (access && promotionStartedAt) {
  ...
  const before = Math.floor((Date.parse(aliasRestoreAt ?? lastSuccess ?? promotionStartedAt) + 5 * 60 * 1000) / 1000);
...
const report = {
  ok: true,
  read_only: true,
  incident_type: "COMMUNICATION_BAD_PROMOTION_MISSING_CRON_ROUTE",
  ...
  first_missed_heartbeat_at: promotionStartedAt,
  alias_restored_at: aliasRestoreAt,
  first_healthy_after_restore_at: lastSuccess,
  ...
  dpl_6saL: {
    ...
    error: dpl6json.error ?? null,
  },
  aliases: prodAlias.map((row) => ({
    alias: row.alias,
    deploymentId: row.deploymentId,
    created: iso(typeof row.created === "number" ? row.created : Date.parse(String(row.created || ""))),
  })),
  ...
  human_requests: (human.data?.payload?.requests ?? []).map((row) => ({ id: row.id, status: row.status, requested_at: row.requested_at, due_at: row.due_at })),
