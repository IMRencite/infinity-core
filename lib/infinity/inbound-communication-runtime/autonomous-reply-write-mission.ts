...
import {
  ...
  AUTONOMOUS_REPLY_WRITE_SOURCE,
  AUTONOMOUS_REPLY_WRITE_VENTURE,
} from "./constants";
import { executeAutonomousReplyWriteVerification } from "./autonomous-reply-write-verification";

export const AUTONOMOUS_REPLY_WRITE_VERIFICATION_MISSION = AUTONOMOUS_REPLY_WRITE_MISSION;
export const AUTONOMOUS_REPLY_WRITE_VERIFICATION_MISSION_ID = AUTONOMOUS_REPLY_WRITE_MISSION_ID;

export const AUTONOMOUS_REPLY_WRITE_STEPS = [
...
export async function executeAutonomousReplyWriteVerificationMission(input: {
  ...
  const emit = {
    ...
    candidateId: null,
    experimentId: null,
    missionId: AUTONOMOUS_REPLY_WRITE_MISSION_ID,
    missionType: AUTONOMOUS_REPLY_WRITE_MISSION,
    source: AUTONOMOUS_REPLY_WRITE_SOURCE,
    technicalDetail: "simulation:FALSE;creProspects:0;maxReplies:1",
  ...
  const instrumented = await runInstrumentedMission({
    emit,
    knownSteps: [...AUTONOMOUS_REPLY_WRITE_STEPS],
    ...
    run: async (handle) => {
      ...
      await handle.step({
        ...
        summary: "Preparing a dedicated autonomous-reply verification conversation isolated from CRE evidence.",
        run: async () => {
          if (phaseMs > 0) await wait(phaseMs);
      ...
      await handle.step({
        ...
        summary: "Reusing an existing controlled thread or sending at most one setup email.",
        run: async () => {
          if (phaseMs > 0) await wait(phaseMs);
      ...
      await handle.step({
        ...
        engine: "performance_intelligence",
        summary: "Reading only the controlled verification thread for a real inbound reply.",
        run: async () => {
          if (phaseMs > 0) await wait(phaseMs);
  ...
  return {
    mission: AUTONOMOUS_REPLY_WRITE_MISSION,
    missionId: AUTONOMOUS_REPLY_WRITE_MISSION_ID,
    ...
    hq: {
      ...
      latestCompleted:
        instrumented.view.latestCompleted?.missionType === AUTONOMOUS_REPLY_WRITE_MISSION
          ? ("PASS" as const)
          : ("FAIL" as const),
    },
